...
namespace MMDK.Core
{
    ...
    class MoneyManager
    {
        ...
        public void init(string _unitName, string _path)
        {
            unitName = _unitName;
            ...
            // read users
            // wallet
            var lines = FileHelper.readLines(path + walletf);
            ...
            // benefit
            lines = FileHelper.readLines(path + benefitf);
            foreach (var line in lines)
            {
        ...
        void save()
        {
            lock (btcMutex)
            {
                try
                {
                    ...
                    FileHelper.writeText(path + walletf, sbwallet.ToString());
                    FileHelper.writeText(path + benefitf, sbbenefit.ToString());
                }
                catch (Exception ex)
                {
                    FileHelper.Log(ex);
        ...
        void writeRecord(BTCRecord record)
        {
            lock (recordMutex)
            {
                try
                {
                    FileHelper.appendText(path + recordf, record.ToString());
                }
                catch (Exception ex)
                {
                    FileHelper.Log(ex);
        ...
        public string transMoney(long fromqq, long targetqq, long money)
        {
            ...
            try
            {
                ...
                res = $"您向{user2.UserId}发起转账{money}枚{MoneyManager.unitName}，";
        ...
        public long addMoney(long money)
        {
            try
            {
                lock (moneyMutex)
                {
                    ...
                    if (realMoney != money)
                    {
                        FileHelper.Log($"{UserId}的{MoneyManager.unitName}溢出，{_wallet.money}+{money}");
                    }
    ...
    class BTCBenefit
    {
        ...
        public void parse(string line)
        {
            try
            {
                ...
                if (items.Length >= 3)
                {
                    ...
                    lastGetTime = UtilHelper.toDateTime(items[2]);// DateTime.ParseExact(items[2], "yyyy-MM-dd HH:mm:ss", System.Globalization.CultureInfo.CurrentCulture);
                }
            }
            catch (Exception ex)
            {
        ...
        public override string ToString()
        {
            try
            {
                return $"{uid}\t{sum}\t{UtilHelper.toTimestamp(lastGetTime)}";
            }
            catch (Exception ex)
            {
                FileHelper.Log(ex);
