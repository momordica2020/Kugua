...
namespace MMDK.Plugins
{
    class TranslatePlugin : Plugin
    {
        ...
        public TranslatePlugin() : base("Translate")
        {
        ...
        public override void InitSource()
        {
            try
            {
                ...
                var lines = FileHelper.readLines(PluginPath + countryListName);
        ...
        public string Translation(string src, string to = "简体中文", string from = "自动")
        {
            ...
            //return url;
            string httpresult = WebHelper.getData(url, Encoding.UTF8, "", true);
            FileHelper.Log(httpresult);
            string res = "";
            try
            {
        ...
        void InitTkk()
        {
            ...
            try
            {
                tkkHtml = WebHelper.getData(url, Encoding.UTF8, "", true);
                tkk = GetTkk(tkkHtml);
            }
            catch (Exception ex)
            {
