using Kugua.Data.ConfigStructs;
...
namespace Kugua.Core
{
    ...
    public class Config
    {
        ...
        readonly string CONFIG_FILE_NAME = "config.json";
        string configFile;
        ...
        public bool Load()
        {
            ...
            lock (loadMutex)
            {
                ...
                try
                {
                    configFile = $"{Directory.GetCurrentDirectory()}{Path.DirectorySeparatorChar}{CONFIG_FILE_NAME}";
                    ...
                    App = JsonConvert.DeserializeObject<AppConfig>(jsonString);
                    //SaveConfig();
        ...
        public string RootPath
        {
            get
            {
                if (App != null && !string.IsNullOrWhiteSpace(App.ResoucePath))
                {
                    return $"{Directory.GetCurrentDirectory()}{Path.DirectorySeparatorChar}{App.ResoucePath}{Path.DirectorySeparatorChar}";
                }
        ...
        public string FullPath(string Name)
        {
            try
            {
                if (App != null && App.Resouces != null)
                {
                    if (App.Resouces.TryGetValue(Name, out ConfigResouce res))
                    {
                        string fullPath = $"{RootPath}{res.Path.Replace("/","\\")}";
                        //string fullFilePath = System.IO.Path.GetFullPath(fullPath);
                        if (res.Type == ConfigResouceType.Path && !Directory.Exists(fullPath))
                        {
                            Logger.Log($"新建资源文件夹，路径是{fullPath}");
                            Directory.CreateDirectory(fullPath);
                        }
                        if (res.Type==ConfigResouceType.File && !File.Exists(fullPath))
                        {
